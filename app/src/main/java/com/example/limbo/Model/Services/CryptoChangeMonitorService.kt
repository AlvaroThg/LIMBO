...
class CryptoChangeMonitorService(
    ...
    private fun checkSignificantChange(
        ...
        if (lastPrice == 0f) return  // Skip first execution when we don't have a previous value
        ...
        when {
            Math.abs(percentChange) >= largeChangeThreshold -> {
                ...
                val message = "IMPORTANT ALERT! USDT/BOB price on $exchange has $direction by ${String.format("%.2f", Math.abs(percentChange))}% (from ${String.format("%.2f", lastPrice)} to ${String.format("%.2f", currentPrice)})"
                sendNotification(exchange, message, currentPrice, "large_change", isIncrease)
            }
            Math.abs(percentChange) >= mediumChangeThreshold -> {
                val direction = if (isIncrease) "increased" else "decreased"
                val message = "Notable change: USDT/BOB price on $exchange has $direction by ${String.format("%.2f", Math.abs(percentChange))}% (from ${String.format("%.2f", lastPrice)} to ${String.format("%.2f", currentPrice)})"
                sendNotification(exchange, message, currentPrice, "medium_change", isIncrease)
            }
            Math.abs(percentChange) >= smallChangeThreshold -> {
                val direction = if (isIncrease) "increased" else "decreased"
                val message = "Update: USDT/BOB price on $exchange has $direction by ${String.format("%.2f", Math.abs(percentChange))}% (from ${String.format("%.2f", lastPrice)} to ${String.format("%.2f", currentPrice)})"
    ...
    private fun sendNotification(
        ...
        val notificationBuilder = NotificationCompat.Builder(context, CHANNEL_ID)
            .setSmallIcon(icon)
            .setContentTitle("Price change on $exchange")
        ...
        // Add vibration for medium and large changes
